...
const OrderDetailsPage = () => {
  ...
  const generatePDF = () => {
    ...
    doc.text('LOURDES TEXTILES PVT. LTD', 105, 27);
    doc.text('19B, JAWAHARLAL NEHRU ROAD, NEW MARKET,', 105, 32);
    doc.text('Kolkata,', 105, 37);
    doc.text('GSTIN/UIN: 19AAACL5546P1ZW', 105, 42);
    doc.text('State Name: West Bengal, Code: 19', 105, 47);

    doc.text(`Invoice No.: ${orderDetails._id}`, 14, 60);
    doc.text(`Dated: ${new Date(orderDetails.createdAt).toLocaleDateString()}`, 105, 60);
    ...
    const subtotal = orderDetails.orderItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
    ...
    doc.setFontSize(9);
    doc.text("We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct.", 14, baseY + 43);
