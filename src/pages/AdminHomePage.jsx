...
const AdminHomePage = () => {
  ...
  useEffect(() => {
    ...
    const trendArray = Object.keys(trendMap).map(date => ({ date, orders: trendMap[date] }));
...
const countUsers = useCountUp(userCount);
  ...
  <div className="p-6 rounded-2xl bg-white/70 shadow-xl backdrop-blur border border-green-100 hover:scale-[1.02] hover:shadow-2xl transition-all">
    ...
    <Link to="/admin/orders" className="text-sm text-green-600 hover:underline font-medium">
      Manage Orders →
    </Link>
  </div>
  ...
  <div className="p-6 rounded-2xl bg-white/70 shadow-xl backdrop-blur border border-purple-100 hover:scale-[1.02] hover:shadow-2xl transition-all">
    ...
    <Link to="/admin/products" className="text-sm text-purple-600 hover:underline font-medium">
      Manage Products →
    </Link>
  </div>
  ...
  <div className="p-6 rounded-2xl bg-white/70 shadow-xl backdrop-blur border border-yellow-100 hover:scale-[1.02] hover:shadow-2xl transition-all">
    ...
    <Link to="/admin/users" className="text-sm text-yellow-600 hover:underline font-medium">
      Manage Users →
    </Link>
  </div>
</div>
      ...
      {/* Order Trend Chart */}
      <div className="mt-10">
        <h2 className="text-2xl font-bold mb-4">Order Trends</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm text-left text-gray-700">
              ...
              <tbody>
                ...
                {users.length === 0 && (
                  <tr>
                    <td colSpan={3} className="py-6 px-6 text-center text-gray-500 italic">
                      No users found.
                    </td>
                  </tr>
                )}
