                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="hidden md:flex flex-col space-y-4">
                        {selectedProduct.images.map((image, index) => (
                            <img
                                ...
                                className={`w-20 h-20 rounded-xl cursor-pointer border transition-all duration-300 ${mainImage === image.url ? "border-sky-600 shadow-lg scale-105" : "border-gray-300 hover:border-sky-400"}`}
                    ...
                    <div className="md:w-1/2 flex flex-col justify-between">
                        <div>
                            ...
                            <div className="text-2xl font-semibold text-sky-700 mb-4">${selectedProduct.price}</div>
                                ...
                                <div className="flex gap-3">
                                    {selectedProduct.colors.map((color) => (
                                        <button
                                            ...
                                            key={color}
                                            className={`w-9 h-9 rounded-full border transition-all duration-300 ${selectedColor === color ? "border-4 border-sky-600 scale-110" : "border-gray-300 hover:border-gray-500"}`}
                                ...
                                <div className="flex gap-3">
                                    {selectedProduct.sizes.map((size) => (
                                        <button
                                            ...
                                            key={size}
                                            className={`px-4 py-2 rounded-xl border transition-all duration-200 font-medium ${selectedSize === size ? "bg-sky-600 text-white" : "border-gray-300 hover:bg-sky-50"}`}
                                        >
                                            {size}
                                        </button>
                                    ))}
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex items-center gap-4">
                                    <button onClick={() => handleQuantityChange("minus")} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-md">-</button>
                                    <span className="text-lg">{quantity}</span>
                                    <button onClick={() => handleQuantityChange("plus")} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-md">+</button>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className={`relative group overflow-hidden rounded-full py-3 px-8 w-full text-white font-semibold transition-all duration-300 ${
                                    isButtonDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-sky-600 hover:bg-sky-700"
                                }`}
                            >
                        ...
                        {/* Characteristics */}
                        <div className="mt-10">
                            <h3 className="text-lg font-semibold mb-2 text-gray-700">Characteristics</h3>
